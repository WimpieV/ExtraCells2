...
public class ItemPartECBase extends ItemECBase implements IPartItem, IItemGroup, IColoredItem {
	...
	public String getItemStackDisplayName(ItemStack stack) {
		...
		if (stack.getItemDamage() == PartEnum.INTERFACE.ordinal()) {
			return ECApi.instance().blocks().blockInterface().maybeItem().get().getItemStackDisplayName(ECApi.instance().blocks().blockInterface().maybeStack(1).get());
		}
	...
	public EnumRarity getRarity(ItemStack itemStack) {
		if (itemStack != null && itemStack.getItemDamage() == PartEnum.OREDICTEXPORTBUS.ordinal()) {
	...
	public String getUnlocalizedGroupName(Set<ItemStack> otherItems, ItemStack itemStack) {
		return PartEnum.values()[MathHelper.clamp(itemStack.getItemDamage(), 0, PartEnum.values().length - 1)].getGroupName();
	}

	@Override
	public String getUnlocalizedName(ItemStack itemStack) {
		return PartEnum.values()[MathHelper.clamp(itemStack.getItemDamage(), 0, PartEnum.values().length - 1)].getUnlocalizedName();
	}
