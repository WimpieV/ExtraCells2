...
public class ItemStorageCellPhysical extends ItemECBase implements IStorageCell, IAEItemPowerStorage, IEnergyContainerItem {
	...
	public void addInformation(ItemStack itemStack, EntityPlayer player, List list, boolean par4) {
		...
		list.add(String.format(I18n.translateToLocal("extracells.tooltip.storage.physical.bytes"), usedBytes, cellInv.getTotalBytes()));
		list.add(String.format(I18n.translateToLocal("extracells.tooltip.storage.physical.types"), cellInv.getStoredItemTypes(), cellInv.getTotalItemTypes()));
		if (usedBytes > 0) {
			list.add(String.format(I18n.translateToLocal("extracells.tooltip.storage.physical.content"), cellInv.getStoredItemCount()));
		}
	}

	@Override
	public int getBytesPerType(ItemStack cellItem) {
		return ECConfigHandler.dynamicTypes ? bytes_cell[MathHelper.clamp(cellItem.getItemDamage(), 0, suffixes.length - 1)] / 128 : 8;
	}
	...
	public int getBytes(ItemStack cellItem) {
		return bytes_cell[MathHelper.clamp(cellItem.getItemDamage(), 0, suffixes.length - 1)];
	}
	...
	public String getItemStackDisplayName(ItemStack stack) {
		...
		if (stack.getItemDamage() == 4) {
			try {
				...
				if (list.isEmpty()) {
					return super.getItemStackDisplayName(stack) + " - " + I18n.translateToLocal("extracells.tooltip.empty1");
				}
	...
	public void getSubItems(Item item, CreativeTabs creativeTab, List itemList) {
		for (int i = 0; i < suffixes.length; i++) {
			...
			if (i == 4) {
				...
				s.getTagCompound().setDouble("power", MAX_POWER);
				itemList.add(s);
			}
		}
	}
