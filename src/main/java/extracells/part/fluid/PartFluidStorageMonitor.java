...
public class PartFluidStorageMonitor extends PartECBase implements IStackWatcherHost {
	...
	public List<String> getWailaBodey(NBTTagCompound data, List<String> list) {
		...
		if (fluid != null) {
			list.add(I18n.translateToLocal("extracells.tooltip.fluid") + ": " + fluid.getLocalizedName(new FluidStack(fluid, Fluid.BUCKET_VOLUME)));
			if (isActive()) {
				list.add(I18n.translateToLocal("extracells.tooltip.amount") + ": " + amount + "mB");
			}
			else {
				list.add(I18n.translateToLocal("extracells.tooltip.amount") + ": 0mB");
			}
		}
		else {
	...
	public boolean onActivate(EntityPlayer player, EnumHand hand, Vec3d pos) {
		...
		if (s == null) {
			...
			if (watcher != null) {
				watcher.remove(FluidHelper.createAEFluidStack(fluid));
			}
			fluid = null;
			amount = 0L;
		...
		if (!FluidHelper.isEmpty(s)) {
			...
			fluid = FluidHelper.getFluidFromContainer(s).getFluid();
			if (watcher != null) {
				watcher.add(FluidHelper.createAEFluidStack(fluid));
			}
			IPartHost host = getHost();
			if (host != null) {
				host.markForUpdate();
	...
	public void renderDynamic(double x, double y, double z, float partialTicks, int destroyStage) {
		...
		IAEFluidStack aeFluidStack = FluidHelper.createAEFluidStack(fluid);
		aeFluidStack.setStackSize(amount);
		if (aeFluidStack != null) {
			GlStateManager.pushMatrix();
			GlStateManager.translate(x + 0.5, y + 0.5, z + 0.5);

			GlStateManager.glNewList((Integer) dspList, GL11.GL_COMPILE_AND_EXECUTE);
	...
	public void updateWatcher(IStackWatcher w) {
		watcher = w;
		if (fluid != null) {
			w.add(FluidHelper.createAEFluidStack(fluid));
		}
